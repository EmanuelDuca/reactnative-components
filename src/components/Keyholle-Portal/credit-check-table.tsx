...
import { CreditCheckResponse, CreditChecksResponse } from "~/api/credit-checks";
import { useLocalizationContext } from "~/contexts/LocalizationContext/LocalizationContext";
import {
  CreditCheckRemarksTable,
  creditCheckRemarksTableColumns,
} from "~/tables/credit-checks-table";
    ...
    <View key={row.id} className="bg-accent">
      ...
      {row.original.remarks.length > 0 && (
        <View className="border-b border-border p-4">
          <CreditCheckRemarksTable table={remarksTable} isPending={isPending} />
        </View>
      )}
    </View>
  );
  ...
  return (
    <AccordionTableRow value={row.id}>
      <AccordionTrigger iconClassName="hidden">
        <TableRow className={cn("flex flex-col")}>
    ...
    // <AccordionItem key={row.id} value={row.id} className="bg-accent">
    //   <AccordionTrigger iconClassName="top-3 right-4">
    //     <RowContent />
    //   </AccordionTrigger>
    //   <AccordionContent>
    //     {row.original.remarks.length > 0 && (
...
>[] = [
  ...
  },
  {
    ...
    header: ({ column }) => {
      const { t } = useLocalizationContext();
      return (
        <TableHead column={column} className="min-w-[170px] flex-1 justify-end">
    ...
    cell: ({ row }) => {
      const { t } = useLocalizationContext();
      return (
        <TableCell className="min-w-[170px] flex-1 justify-end">
          <TableCellText>
